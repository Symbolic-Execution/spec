...
Reader keys are generated by `sym-client` and used to decrypt
reader-targeted ciphertexts returned to the user.
...
- submit signed disclosure requests and receive `ReaderCiphertextV1` when ready
- poll asynchronous request status when a disclosure request returns pending
